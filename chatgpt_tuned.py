...
class TunedGPT(AbstractFunction):
    """
    Arguments:
    ...
    Example Usage:
        ...
        If the user wants to retrieve the summary of each video, the TunedGPT function can be used as:

            query = "Generate the summary of the video"
            cursor.table("video_transcripts").select(f"TunedGPT({query}, text)")
        ...
        along with each query, a prompt can be set as such:
            ...
            cursor.table("video_transcripts").select(f"TunedGPT({query}, text, {prompt})")
